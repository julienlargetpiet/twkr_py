...
def ns(input_l, dim_end=1, strt_l=[], rtn_l=[], id_rec_main=0, wrk_l=None, flag_l=[]):
    ...
    def end_(input_l, rtn_l, flag_l, dim_end):
        ...
        for el in range(len(flag_l)):
            ...
            if el == 0:
                ...
                idx += len(nestfind(input_l, flag_l[el])) + flag_l[el][-1]

            elif len(flag_l[el]) == len(flag_l[el - 1]):

                idx += len(nestfind(input_l, flag_l[el])) + (flag_l[el][-1] - flag_l[el - 1][-1]) - 1

            elif len(flag_l[el]) > len(flag_l[el - 1]): 

                idx += len(nestfind(input_l, flag_l[el])) - (len(nestfind(input_l, flag_l[el - 1])) - flag_l[el][-1])
                ...
                for i in range(1, len(flag_l[el - 1]) - len_flag_l_curr + 1):
                    ...
                    remain_idx += len(nestfind(input_l, flag_l[el - 1][0:len(flag_l[el - 1]) - i])) - flag_l[el - 1][-i] - 1

                idx += len(nestfind(input_l, flag_l[el])) + (flag_l[el][-1] - flag_l[el - 1][len_flag_l_curr - 1]) - 1 + remain_idx 

            if len(flag_l[el]) == dim_end:

                cur = nestfind(input_l, flag_l[el])
            ...
            if strt_l[-1] + 1 == len(wrk_l_pre[-1]):
                ...
                if len(strt_l) == 1:
                    ...
                    if id_rec_main == len(input_l):
                        ...
                        return end_(input_l, rtn_l, flag_l, dim_end)
    ...
    if len(strt_l) <= dim_end and len(strt_l) > 0 and type(nestfind(input_l, strt_l)) == list:
    ...
    return ns(input_l, dim_end, strt_l, rtn_l, id_rec_main, wrk_l, flag_l)
